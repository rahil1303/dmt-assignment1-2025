...
def run_case2_decision_tree_log():
    X, df = load_dataset("/content/drive/MyDrive/Data Mining Techniques/Task-4/advanced_preprocessed_dataset.csv")
...
def run_case3_decision_tree_log():
    X, df = load_dataset("/content/drive/MyDrive/Data Mining Techniques/Task-4/advanced_preprocessed_dataset.csv")
...
def run_decision_tree_training_vs_cv():
    X, df = load_dataset("/content/drive/MyDrive/Data Mining Techniques/Task-4/advanced_preprocessed_dataset.csv")
    print("\n[Log Target] Training vs CV Error Check")
    evaluate_model_with_training(DecisionTreeRegressor(max_depth=5, min_samples_leaf=10, random_state=42), X, df['ROOM_ESTIMATE_LOG'], target_name="ROOM_ESTIMATE_LOG")
    print("\n[Clean Target] Training vs CV Error Check")
    evaluate_model_with_training(DecisionTreeRegressor(max_depth=5, min_samples_leaf=10, random_state=42), X, df['ROOM_ESTIMATE_CLEANED'], target_name="ROOM_ESTIMATE_CLEANED")

def run_case3_decision_tree_multioutput():
    X, df = load_dataset("/content/drive/MyDrive/Data Mining Techniques/Task-4/advanced_preprocessed_dataset.csv")
    Y = df[['STRESS_LEVEL_CLEANED', 'ROOM_ESTIMATE_CLEANED']]

    model = MultiOutputRegressor(DecisionTreeRegressor(max_depth=5, min_samples_leaf=10, random_state=42))
    model.fit(X, Y)
