...
def evaluate_model(model, X, y, target_name):
    ...
    print(f"\nLinear Regression on {target_name}:")
...
def run_case1_linear_regression():
...
def run_case3_linear_regression_log():
    ...
    for target in Y.columns:
        y = Y[target]
        evaluate_model(LinearRegression(), X, y, target_name=target)

def run_linear_regression_training_vs_cv():
    X, df = load_dataset("/content/drive/MyDrive/Data Mining Techniques/Task-4/advanced_preprocessed_dataset.csv")
    print("\n[Log Target] Training vs CV Error Check")
    evaluate_model_with_training(LinearRegression(), X, df['ROOM_ESTIMATE_LOG'], target_name="ROOM_ESTIMATE_LOG")
    print("\n[Clean Target] Training vs CV Error Check")
    evaluate_model_with_training(LinearRegression(), X, df['ROOM_ESTIMATE_CLEANED'], target_name="ROOM_ESTIMATE_CLEANED")
